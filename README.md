...
On first standard-mode launch, an admin is created only from explicit `SUPERADMIN_USERNAME`, `SUPERADMIN_EMAIL`, and `SUPERADMIN_PASSWORD` settings. The password must contain at least 16 characters. There are no built-in production credentials. Remove the bootstrap password from the deployment environment after provisioning.
...
| `JWT_SECRET` | Yes | Random secret of at least 32 bytes; no fallback |
...
| `SUPERADMIN_USERNAME` | First standard bootstrap | Initial admin username |
| `SUPERADMIN_PASSWORD` | First standard bootstrap | Initial admin password, at least 16 characters |
| `SUPERADMIN_EMAIL` | First standard bootstrap | Initial admin email |
...
```bash
# Configure Docker keys and explicit admin credentials first
cp .env.example .env
# Fill .env, then start standard mode
...
**Data notice:** The showcase uses synthetic schools, students, parents, locations, identifiers, and attendance records generated by `seed-demo.ts`. They are demonstration data, not real school/student records. Screenshots are intended to illustrate that demo. Generated phone numbers must never be used to contact anyone.
...
Demo endpoints require `DEMO_MODE=true`. Seeded accounts are marked `isDemoAccount` and cannot authenticate outside demo mode; reset existing demo data once after upgrading to apply that marker. Mock OTP responses/logs are demo-only. SMS delivery remains a demo implementation and needs a real provider for operational use. Destructive demo seed/reset also requires a MongoDB database whose name contains a `demo` segment (for example `SBTS_DEMO`). Never point the demo deployment at a real database. Generate JWT and encryption keys separately with `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`. For local `npm run demo`, create `backend/.env.demo` with these keys, `DEMO_MODE=true`, and a dedicated `MONGO_URI`.
...
This project is licensed under the [MIT License](LICENSE), which permits commercial reuse, modification, and redistribution while requiring the copyright and license notice to be retained. Public visibility is separate from licensing. Third-party dependencies and assets retain their own licenses.
