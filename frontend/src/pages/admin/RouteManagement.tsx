...
const RecenterMap = ({ center }: { center: number[] }) => {
  const map = useMap();
  useEffect(() => {
...
const RouteManagement = () => {
    ...
    // Data
    const [routes, setRoutes] = useState<IApiRoute[]>([]);
    const [unassignedStudents, setUnassignedStudents] = useState<IApiStudent[]>([]);
    const [loadingData, setLoadingData] = useState(true);
    
    // Map State
    const [selectedStudents, setSelectedStudents] = useState<IApiStudent[]>([]); // Students added to current route
    const [mapCenter, setMapCenter] = useState<[number, number]>([24.7136, 46.6753]); // Riyadh
    const [routePathGeoJson, setRoutePathGeoJson] = useState<[number, number][]>([]); // Decoded for map [lat, lng] array
    ...
    // --- Interaction ---
    const toggleStudentSelection = (student: IApiStudent) => {
        const isSelected = selectedStudents.find(s => s._id === student._id || s.id === student.id);
        if (isSelected) {
            setSelectedStudents(selectedStudents.filter(s => s._id !== student._id && s.id !== student.id));
    ...
    const handleAutoRoute = async () => {
        ...
        try {
            ...
            if (data.code === 'Ok' && data.trips.length > 0) {
                ...
                const leafletPath = geojsonCoords.map((c: [number, number]) => [c[1], c[0]]);
                setRoutePathGeoJson(leafletPath as [number, number][]);
    ...
    const handleSaveRoute = async () => {
        ...
        try {
            ...
            await api.post('/routes', {
                name: routeName,
                students: selectedStudents.map(s => s._id || s.id),
            ...
            setError(axiosErr.response?.data?.message || t('routeManagement.errors.saveFailed'));
        } finally {
                    ...
                    <div className="w-full lg:w-2/3 bg-gray-100 rounded-2xl border border-gray-200 overflow-hidden relative shadow-inner z-0">
                        <MapContainer center={mapCenter} zoom={12} style={{ height: '100%', width: '100%' }} className="z-0">
                            ...
                            {unassignedStudents.map(s => {
                                ...
                                const isSelected = selectedStudents.some(sel => sel._id === s._id || sel.id === s.id);
                                if (isSelected) return null; // Don't show generic pin if selected
                            ...
                            {selectedStudents.map((s) => {
                                ...
                                const lng = s.location!.coordinates[0];
                                return (
                                    <Marker key={`sel-${s.id}`} position={[lat, lng]} icon={selectedIcon}>
