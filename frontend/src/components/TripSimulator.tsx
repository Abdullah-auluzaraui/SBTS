...
const APPROACH_THRESHOLD = 400; // metres ΓÇö pulsing amber ring, sim continues
const BOARDING_THRESHOLD = 150; // metres ΓÇö NFC fire (raised: OSRM waypoints land on road, not doorstep)
...
const TripSimulator = ({
    routePath,         // [[lat, lng]] ΓÇö already swapped by DriverDashboard from OSRM
    students,          // ALL currently unboarded students [{_id, name, location:{coordinates:[lng,lat]}}]
    activeStudent,     // kept for backward compat ΓÇö NOT used for proximity
    busId,
    ...
    onCurrentStudent,  // fn(student | null) ΓÇö null initially, set when bus enters 50 m radius
}) => {
    ...
    // ΓöÇΓöÇ Mutable refs (avoid stale closures inside setInterval) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // Prop mirrors ΓÇö always-fresh inside the perpetual interval
    ...
    // ΓöÇΓöÇ On routePath arrival: snap bus to start of route ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    useEffect(() => {
    ...
    // ΓöÇΓöÇ tickRef: updated every render ΓÇö always has fresh prop closures ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    tickRef.current = () => {
        ...
        const [aLat, aLng] = routePath[prevIdx]; // previous position ΓÇö forms AΓåÆB segment
        ...
        // 3. Global spatial scan ΓÇö check EVERY unboarded student, not just the route-order one
        const unboarded = studentsRef.current.filter(
            s => s.location?.coordinates && !boardedRef.current.has(String(s._id))
        );

        // 3a. Boarding check: segment AΓåÆB sweep catches students even when nodes are sparse
        ...
        if (boardingTarget) {
            ...
            // Fire-and-forget ΓÇö interval is NEVER paused or stopped here
            ...
            // Clear the UI flash after 1.5 s ΓÇö does NOT affect interval control
            setTimeout(() => {
                if (!mountedRef.current) return;
            ...
            // NO return ΓÇö bus continues advancing along the route
        }
        ...
        if (idx >= total - 1) {
            ...
            // Auto-resolve any student still not processed ΓÇö last-mile safety net.
    ...
    // ΓöÇΓöÇ Perpetual interval ΓÇö created ONCE on mount, NEVER recreated ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
