...
const PROFILE_PATHS: Partial<Record<UserRole, string>> = {
  ...
  schooladmin: '/admin/profile',
};

const MainLayout: React.FC<MainLayoutProps> = ({ children, onRefreshRequired }) => {
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  const { t, i18n } = useTranslation();
  const [isLogoutModalOpen, setLogoutModalOpen] = useState(false);
  const [isDropdownOpen, setDropdownOpen]       = useState(false);
  const { notifications, unreadCount, markAsRead, markAllAsRead, toastData, setToastData } =
    useNotifications({ onRefreshRequired });

  const profilePath = user ? PROFILE_PATHS[user.role] : undefined;
      ...
      <header className="bg-white border-b border-gray-200 shadow-sm py-4 px-6 md:px-12 flex justify-between items-center sticky top-0 z-50">
        ...
        {user && (
          <div className="flex items-center gap-3 text-sm font-semibold">
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <h4 className={`text-sm font-bold truncate ${n.type === 'urgent_alert' ? 'text-red-700' : 'text-gray-800'}`}>
                                  {(n as any).notificationType ? t(`notifications.${(n as any).notificationType}.title`) : (n as any).title}
                                </h4>
                                <p className="text-xs text-gray-600 mt-0.5 leading-relaxed">
                                  {(n as any).notificationType ? t(`notifications.${(n as any).notificationType}.body`) : n.message}
                                </p>
            ...
            {(['parent', 'driver', 'schooladmin'] as UserRole[]).includes(user.role) && profilePath && (
              <button
            ...
            <div className="flex items-center gap-4 z-10">
              ...
              <div>
                <h4 className={`font-bold text-sm ${toastData.type === 'urgent_alert' ? 'text-red-700' : 'text-gray-800'}`}>
                  {(toastData as any).notificationType ? t(`notifications.${(toastData as any).notificationType}.title`) : (toastData as any).title}
                </h4>
                <p className="text-gray-500 text-xs mt-0.5 leading-relaxed">
                  {(toastData as any).notificationType ? t(`notifications.${(toastData as any).notificationType}.body`) : toastData.message}
                </p>
              </div>
            </div>
            <button
