...
interface RelinkParentModalProps {
  student: StudentSummary;
  onClose?: () => void;
  onSuccess?: (updatedStudent: unknown) => void;
}

const RelinkParentModal: React.FC<RelinkParentModalProps> = ({ student, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!password.trim()) { setError(t('studentManagement.confirmPasswordLabel')); return; }
    setError('');
    setSubmitting(true);
    try {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden max-h-[90vh]" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between gap-3 bg-primary-50/60 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 shrink-0 bg-primary-100 rounded-xl flex items-center justify-center"><Link2 size={20} className="text-primary-600" /></div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 min-h-0 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="space-y-1.5">
            <label htmlFor="relink-password" className="block text-gray-700 font-bold text-sm px-1">{t('studentManagement.confirmPasswordLabel')}</label>
            <div className="relative">
              <input id="relink-password" type={showPassword ? 'text' : 'password'} value={password}
                ...
                className="w-full ps-4 pe-11 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all" />
