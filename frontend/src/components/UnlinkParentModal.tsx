...
interface UnlinkParentModalProps {
  student: StudentSummary;
  onClose?: () => void;
  onSuccess?: (updatedStudent: unknown) => void;
}

const UnlinkParentModal: React.FC<UnlinkParentModalProps> = ({ student, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => !submitting && onClose?.()}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden max-h-[90vh]" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between gap-3 bg-red-50/60 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 shrink-0 bg-red-100 rounded-xl flex items-center justify-center"><ShieldAlert size={20} className="text-red-600" /></div>
          ...
          <button type="button" onClick={() => !submitting && onClose?.()} disabled={submitting}
            className="w-9 h-9 shrink-0 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-600 transition-colors disabled:opacity-50" aria-label={t('common.close')}>
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 min-h-0 overflow-y-auto px-6 py-5 space-y-5">
          <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 shrink-0 rounded-full bg-white border border-gray-200 flex items-center justify-center font-bold text-gray-500">{student?.name?.charAt(0) || '?'}</div>
            ...
            {student?.parentName && (
              <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between gap-3">
                <span className="text-xs text-gray-500 font-semibold">{t('studentManagement.linkedParentLabel')}</span>
                <span className="text-xs font-bold text-gray-700 bg-white px-2 py-1 rounded-lg border border-gray-200 truncate">{student.parentName}</span>
              </div>
            )}
          </div>
          ...
          <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100">
            <div className="flex items-start gap-2 mb-3">
              ...
              <p className="text-sm font-bold text-amber-800">{t('studentManagement.unlinkWarningHeading')}</p>
            </div>
            <ul className="space-y-2 ps-6 list-disc marker:text-amber-500 text-xs text-amber-900 leading-relaxed">
          ...
          <div className="space-y-1.5">
            <label htmlFor="unlink-password" className="block text-gray-700 font-bold text-sm px-1">{t('studentManagement.confirmPasswordLabel')}</label>
            <div className="relative">
              <input id="unlink-password" type={showPassword ? 'text' : 'password'} value={password}
                onChange={e => { setPassword(e.target.value); setError(''); }}
                placeholder={t('studentManagement.confirmPasswordPlaceholder')}
                autoComplete="current-password" autoFocus disabled={submitting}
                className="w-full ps-4 pe-11 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all" />
