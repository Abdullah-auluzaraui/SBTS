...
interface NoReceiverModalProps {
  student?: Student;
  parentPhone?: string;
  parentName?: string;
...
const NoReceiverModal: React.FC<NoReceiverModalProps> = ({
  student,
  parentPhone,
  parentName,
  schoolContacts,
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <div className="space-y-3">
            {parentPhone ? (
              <a href={`tel:${parentPhone}`} className="flex items-center gap-3 p-4 bg-green-50 border border-green-100 rounded-xl hover:bg-green-100 transition-colors w-full">
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between gap-3 shrink-0">
          <button type="button" onClick={handleClose} className="px-5 py-2.5 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-colors text-sm">
            {t('common.cancel')}
          </button>
          <button
            type="button"
